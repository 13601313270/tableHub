...
if ($_POST['function'] === 'updateTdValue') {
    ...
    exit;
} elseif ($_POST['function'] === 'updateHeight') {
    $tableInfo = excel::create()->getInfoById(intval($_POST['fileId']));
    $rownum = $_POST['row'];
    if (is_array($tableInfo['data'][intval($_POST['tableNum'])]->row) && count($tableInfo['data'][intval($_POST['tableNum'])]->row) == 0) {
        $tableInfo['data'][intval($_POST['tableNum'])]->row = json_decode('{}');
    }
    if (isset($tableInfo['data'][intval($_POST['tableNum'])]->row->$rownum)) {
        $tableInfo['data'][intval($_POST['tableNum'])]->row->$rownum->height = $_POST['height'];
    } else {
        $tableInfo['data'][intval($_POST['tableNum'])]->row->$rownum = json_decode('{"height":' . $_POST['height'] . '}');
    }
    $result = excel::create()->update('id=' . intval($_POST['fileId']), array(
        'data' => json_encode($tableInfo['data'])
    ));
