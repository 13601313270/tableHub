...
{
    $classAutoLoad = array(
        'datasourceInterface' => 'include/datasourceInterface.php',
    );
    if (isset($classAutoLoad[$model])) {
        include_once($classAutoLoad[$model]);
    } elseif ($model == 'Memcache') {
        throw new Exception('环境不支持memcache');
    } elseif (strpos($model, 'kod_') === false && strpos($model, 'Smarty_') === false && strpos($model, 'datasource') === 0) {
        include_once('include/datasource/' . $model . '.php');
